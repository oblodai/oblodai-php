...
- **One API key.** `publicId:`/`secret:` (or `OBLODAI_PUBLIC_ID`/`OBLODAI_SECRET`) sign every route
  the gateway gates — payments, payouts, settings, documents alike. `adminToken:` is a separate
  thing entirely: it is a self-hosted gateway's provisioning token, sent only on `merchants->*`.
